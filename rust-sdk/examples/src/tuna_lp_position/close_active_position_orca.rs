...
pub async fn close_active_position_orca(rpc: RpcClient, authority: &Keypair, tuna_position_mint: Pubkey) -> Result<()> {
  ...
  let args = CloseActiveTunaLpPositionArgs {
    swap_to_token,
  ...
  let instructions =
    close_active_tuna_lp_position_orca_instructions(&rpc, &authority.pubkey(), &tuna_position_mint, args)?;
  ...
  let tuna_position_address = get_tuna_liquidity_position_address(&tuna_position_mint).0;
  let tuna_position = fetch_tuna_lp_position(&rpc, &tuna_position_address)?;
