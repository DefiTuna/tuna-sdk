...
export default class RepayBadDebt extends BaseCommand {
  ...
  public async run() {
    ...
    const vaultAddress = (await getLendingVaultAddress(args.mint, args.market))[0];
    ...
    const ix = await repayBadDebtInstruction(rpc, signer, vault.address, 0n, vault.data.unpaidDebtShares);
