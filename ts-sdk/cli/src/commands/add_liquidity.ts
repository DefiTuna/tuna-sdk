...
export default class AddLiquidity extends BaseCommand {
  ...
  public async run() {
    const { flags } = await this.parse(AddLiquidity);
    ...
    if (flags.positionMint || flags.positionAddress) {
      ...
      if (market.data.marketMaker == MarketMaker.Fusion) {
        const ixs = await addLiquidityFusionInstructions(rpc, signer, positionMint, args);
        instructions.push(...ixs);
      } else {
        const ixs = await addLiquidityOrcaInstructions(rpc, signer, positionMint, args);
        instructions.push(...ixs);
      }
    } else {
