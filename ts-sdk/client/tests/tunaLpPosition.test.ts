...
import { assertLiquidateTunaLpPosition, liquidateTunaLpPosition } from "./helpers/liquidateTunaLpPosition.ts";
...
import { assertRebalanceTunaLpPosition, rebalanceTunaLpPosition } from "./helpers/rebalanceTunaLpPosition.ts";
import { repayTunaLpPositionDebt } from "./helpers/repayTunaLpPositionDebt.ts";
...
describe("Tuna Liquidity Position", () => {
  ...
  it("Open and close a position ensuring that all token accounts are closed", async () => {
    ...
    await sendTransaction([await closeTunaLpPositionOrcaInstruction(rpc, signer, positionMintKeypair.address)]);
  ...
  it("Set tuna position flags", async () => {
    ...
    await sendTransaction([
      await openTunaLpPositionOrcaInstruction(rpc, signer, positionMintKeypair, whirlpool.address, {
        tickLowerIndex: actualTickIndex - whirlpool.data.tickSpacing,
        tickUpperIndex: actualTickIndex + whirlpool.data.tickSpacing,
    ...
    const tunaPositionAddress = (await getTunaLpPositionAddress(positionMintKeypair.address))[0];
    let tunaPosition = await fetchTunaLpPosition(rpc, tunaPositionAddress);
    expect(tunaPosition.data.flags).toEqual(0);

    // Set flags
    const flags = (1 << 6) - 1; // 111111
    await sendTransaction([
      getSetTunaLpPositionFlagsInstruction({ authority: signer, flags, tunaPosition: tunaPositionAddress }),
    ]);

    tunaPosition = await fetchTunaLpPosition(rpc, tunaPositionAddress);
  ...
  for (const marketMaker of marketMakers) {
    it(`Fails to increase, decrease, collect fees and close a position with non-authority wallet (${MarketMaker[marketMaker]})`, async () => {
      ...
      await closeTunaLpPosition({ rpc, signer, positionMint });
    });
  }
  ...
  for (const marketMaker of marketMakers) {
    it(`Fails to liquidate the position due to: a) unauthorized b) position being healthy (${MarketMaker[marketMaker]})`, async () => {
      ...
      await assert.rejects(liquidateTunaLpPosition({ rpc, signer: LIQUIDATOR_KEYPAIR, positionMint }), err => {
  ...
  for (const marketMaker of marketMakers) {
    it(`Liquidates a position due to an unhealthy state (no bad debt) (${MarketMaker[marketMaker]})`, async () => {
      ...
      assertLiquidateTunaLpPosition(await liquidateTunaLpPosition({ rpc, signer: LIQUIDATOR_KEYPAIR, positionMint }), {
      ...
      await assert.rejects(closeTunaLpPosition({ rpc, signer, positionMint }), err => {
        expect((err as Error).toString()).contain(
  ...
  for (const marketMaker of marketMakers) {
    it(`Can execute openAndIncreaseTunaLpPosition instruction (${MarketMaker[marketMaker]})`, async () => {
      ...
      await openAndIncreaseTunaLpPosition({
        rpc,
        tickLowerIndex: actualTickIndex + pool.data.tickSpacing,
        tickUpperIndex: actualTickIndex + pool.data.tickSpacing * 5,
        pool: pool.address,
  ...
  for (const marketMaker of marketMakers) {
    it(`S/L limit order (${MarketMaker[marketMaker]})`, async () => {
      ...
      const positionMint = await openAndIncreaseTunaLpPosition({
        ...
        tickLowerIndex: actualTickIndex - pool.data.tickSpacing,
        tickUpperIndex: actualTickIndex + pool.data.tickSpacing,
        lowerLimitOrderSqrtPrice: tickIndexToSqrtPrice(actualTickIndex - pool.data.tickSpacing * 2),
        pool: pool.address,
        collateralA: 1_000_000_000n,
        collateralB: 100_000_000n,
        borrowA: 2_000_000_000n,
  ...
  for (const marketMaker of marketMakers) {
    it(`S/L limit order, swap to token B (${MarketMaker[marketMaker]})`, async () => {
      ...
      const positionMint = await openAndIncreaseTunaLpPosition({
        ...
        borrowA: 2_000_000_000n,
        borrowB: 200_000_000n,
        flags: TUNA_POSITION_FLAGS_LOWER_LIMIT_ORDER_SWAP_TO_TOKEN_B,
      });
  ...
  for (const marketMaker of marketMakers) {
    it(`T/P limit order (${MarketMaker[marketMaker]})`, async () => {
      ...
      const positionMint = await openAndIncreaseTunaLpPosition({
        ...
        tickLowerIndex: actualTickIndex - pool.data.tickSpacing,
        tickUpperIndex: actualTickIndex + pool.data.tickSpacing,
        upperLimitOrderSqrtPrice: tickIndexToSqrtPrice(actualTickIndex + pool.data.tickSpacing * 2),
        pool: pool.address,
        collateralA: 1_000_000_000n,
        collateralB: 100_000_000n,
        borrowA: 2_000_000_000n,
  ...
  for (const marketMaker of marketMakers) {
    it(`T/P limit order, swap to token A (${MarketMaker[marketMaker]})`, async () => {
      ...
      const positionMint = await openAndIncreaseTunaLpPosition({
        ...
        borrowA: 2_000_000_000n,
        borrowB: 200_000_000n,
        flags: TUNA_POSITION_FLAGS_UPPER_LIMIT_ORDER_SWAP_TO_TOKEN_A,
      });
  ...
  for (const marketMaker of marketMakers) {
    it(`Re-balance a two-sided position (${MarketMaker[marketMaker]})`, async () => {
      ...
      assertRebalanceTunaLpPosition(
        await rebalanceTunaLpPosition({
          rpc,
          signer,
          positionMint,
          pool: pool.address,
  ...
  for (const marketMaker of marketMakers) {
    it(`Entry price change on the position increase (${MarketMaker[marketMaker]})`, async () => {
      ...
      const positionAfter = await fetchTunaLpPosition(rpc, positionAddress);
    ...
    it(`Increase and decrease position accounting for direct token transfers (${MarketMaker[marketMaker]})`, async () => {
      ...
      assertIncreaseTunaLpPosition(
        ...
        }),
        {
          ...
          vaultBalanceDeltaA: -1000000000n,
          vaultBalanceDeltaB: 0n,
          poolBalanceDeltaA: 2997000000n,
          poolBalanceDeltaB: 399599997n,
          leftoversA: 0n,
          leftoversB: 3n,
        },
      );
      ...
      assertDecreaseTunaLpPositionLiquidity(
        ...
        {
          userBalanceDeltaA: 2755866254n,
          userBalanceDeltaB: 847807082n,
          vaultBalanceDeltaA: 1000000000n,
          vaultBalanceDeltaB: 0n,
          poolBalanceDeltaA: -2755866254n,
          poolBalanceDeltaB: -447807079n,
        },
      );
    });
  }*/
