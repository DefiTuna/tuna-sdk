...
import {
  ...
  MarketMaker,
  rebalanceTunaLpPositionFusionInstructions,
  rebalanceTunaLpPositionOrcaInstructions,
} from "../../src";

import { getPositionAddress } from "./fetch.ts";
import { sendTransaction } from "./mockRpc.ts";

export type RebalanceTunaLpPositionTestArgs = {
...
export type RebalanceTunaLpPositionTestResults = {
  newTickLowerIndex: number;
  newTickUpperIndex: number;
};

export type RebalanceTunaLpPositionTestExpectations = {
  newTickLowerIndex?: number;
  newTickUpperIndex?: number;
};

export function assertRebalanceTunaLpPosition(
  results: RebalanceTunaLpPositionTestResults,
  expectations: RebalanceTunaLpPositionTestExpectations,
) {
...
export async function rebalanceTunaLpPosition({
  rpc,
  signer,
  positionMint,
  pool: poolAddress,
}: RebalanceTunaLpPositionTestArgs): Promise<RebalanceTunaLpPositionTestResults> {
  ...
  if (isOrcaMarket) {
    instructions.push(
      ...(
        await rebalanceTunaLpPositionOrcaInstructions(
          rpc,
          signer,
          positionMint,
          createInstructions,
    ...
    instructions.push(
      ...(
        await rebalanceTunaLpPositionFusionInstructions(
          rpc,
          signer,
          positionMint,
          createInstructions,
