...
import { PUMP_AMM_PROGRAM_ADDRESS } from "@crypticdot/pump-amm-solana-client";
...
import { airdropAddresses, setDefaultFunder } from "./addresses";
...
export async function getTestContext(): Promise<ProgramTestContext> {
  if (_testContext == null) {
    const extraPrograms: Array<[string, Uint8Array]> = [
      ...
      ["../../external_programs/pump_amm", toBytes(PUMP_AMM_PROGRAM_ADDRESS)],
    ];

    const accounts: Array<[Uint8Array, Account]> = [];
    accounts.push([
      toBytes(signer.address),
      new Account(BigInt(100000e9), new Uint8Array(), toBytes(SYSTEM_PROGRAM_ADDRESS), false, 0n),
    ]);
    for (const airdropAddress of airdropAddresses) {
      accounts.push([
        toBytes(airdropAddress.address),
        new Account(BigInt(100000e9), new Uint8Array(), toBytes(SYSTEM_PROGRAM_ADDRESS), false, 0n),
      ]);
    }

    _testContext = await startAnchor("../../", extraPrograms, accounts);
...
export async function sendTransaction(ixs: IInstruction[]) {
  ...
  // DO NOT DELETE THIS CODE. IT IS NEEDED TO RUN ANCHOR TESTS VIA LOCAL VALIDATOR.
  /*
