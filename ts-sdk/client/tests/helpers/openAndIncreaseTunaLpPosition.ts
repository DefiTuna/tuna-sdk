...
export type OpenAndIncreaseLpPositionTestArgs = {
...
export async function openAndIncreaseTunaLpPosition({
  ...
  flags,
  signer = FUNDER,
}: OpenAndIncreaseLpPositionTestArgs) {
  ...
  const feeA =
    (collateralA * BigInt(market.data.protocolFeeOnCollateral) + borrowA * BigInt(market.data.protocolFee)) /
    BigInt(HUNDRED_PERCENT);
  const feeB =
    (collateralB * BigInt(market.data.protocolFeeOnCollateral) + borrowB * BigInt(market.data.protocolFee)) /
    BigInt(HUNDRED_PERCENT);
