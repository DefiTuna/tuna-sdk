...
import {
  ...
  HUNDRED_PERCENT,
  liquidateTunaLpPositionFusionJupiterInstructions,
  liquidateTunaLpPositionOrcaJupiterInstructions,
  MarketMaker,
} from "../../src";

import { fetchPool, fetchPosition, getPositionAddress } from "./fetch.ts";
import { LiquidateTunaLpPositionTestResults } from "./liquidateTunaLpPosition.ts";
import { sendTransaction } from "./mockRpc.ts";

export type LiquidateTunaLpPositionJupiterTestArgs = {
...
export async function liquidateTunaLpPositionJupiter({
  rpc,
  signer,
  positionMint,
  decreasePercent,
  ...
  if (market.data.marketMaker == MarketMaker.Orca) {
    instructions = await liquidateTunaLpPositionOrcaJupiterInstructions(
      signer,
      tunaPosition,
      tunaConfig,
      mintA,
    ...
    instructions = await liquidateTunaLpPositionFusionJupiterInstructions(
      signer,
      tunaPosition,
      tunaConfig,
      mintA,
