...
import { accountExists, getMintDecimals, getPythOraclePriceFeed, loadKeypair } from "examples/utils/common";
...
export async function orcaOpenPositionAndAddLiquidity(): Promise<void> {
   ...
   */
  const nominalCollateral = { a: 1, b: 100 };
  /**
   ...
   */
  const collateral = { a: nominalCollateral.a * decimalsScale.a, b: nominalCollateral.b * decimalsScale.b };
   ...
   */
  const nominalBorrow = {
    a: totalNominalCollateralAmount * (leverage - 1) * borrowRatio.a,
   ...
   */
  const borrow = { a: nominalBorrow.a * decimalsScale.a, b: nominalBorrow.b * decimalsScale.b };
   ...
   */
  const tunaPositionAta: Address = await findAssociatedTokenPda({
  ...
  const addLiquidityOrcaIx = getAddLiquidityOrcaInstruction({
    ...
    vaultA: whirlpoolAccount.data.tokenVaultA,
    vaultB: whirlpoolAccount.data.tokenVaultB,
