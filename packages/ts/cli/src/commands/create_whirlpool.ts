...
export default class CreateWhirlpool extends BaseCommand {
  ...
  static override description = "Create a pool";
  static override examples = [
    "<%= config.bin %> <%= command.id %> address 1 10000",
  ];
  ...
  public async run() {
    ...
    const whirlpoolsConfig = await fetchMaybeFeeTier(
      rpc,
    ...
    const feeTierAddress = (
      await getFeeTierAddress(whirlpoolsConfigAddress, tickSpacing)
    )[0];
    ...
    const tokenBadgeAAddress = (
      await getTokenBadgeAddress(whirlpoolsConfigAddress, mintA.address)
    )[0];

    const tokenBadgeBAddress = (
      await getTokenBadgeAddress(whirlpoolsConfigAddress, mintB.address)
    )[0];

    const initialSqrtPrice = priceToSqrtPrice(
      args.initialPrice,
      mintA.data.decimals,
      mintB.data.decimals
    );
    ...
    if (whirlpool.exists) {
      ...
      throw new Error(
        `Whirlpool already exists at address ${whirlpoolAddress}`
      );
    }

    console.log(
      `Creating whirlpool with tick spacing ${tickSpacing} at address ${whirlpoolAddress}`
    );
